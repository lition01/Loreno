...
<aside class="sidebar" id="sidebar">
  <div class="sidebar-brand">
    ...
    <span class="brand-name">My Account</span>
  </div>
  ...
  <div class="sidebar-footer">
    ...
    <button class="footer-btn" onclick="$('sidebar').classList.remove('open');$('sidebarOverlay').classList.remove('show')">
...
<div class="main-wrapper">
  ...
  <header class="topbar">
    ...
    <div class="topbar-right">
      ...
      <button class="avatar-btn" id="avatarBtn">AM</button>
    </div>
  </header>
...
function seed() {
  if (!localStorage.getItem('orders')) {
    sl('orders', [
      { id: 'ORD-1001', date: '2026-02-15', status: 'Delivered',  total: 289.99, items: [{ n: 'Silk Tie', q: 1, p: 89.99 },{ n: 'Oxford Shirt', q: 2, p: 100.00 }] },
      { id: 'ORD-1002', date: '2026-02-28', status: 'Shipped',    total: 459.00, items: [{ n: 'Wool Blazer', q: 1, p: 459.00 }] },
      { id: 'ORD-1003', date: '2026-03-01', status: 'Processing', total: 120.49, items: [{ n: 'Leather Belt', q: 1, p: 79.00 },{ n: 'Pocket Square', q: 3, p: 13.83 }] },
      { id: 'ORD-1004', date: '2025-12-10', status: 'Delivered',  total: 195.00, items: [{ n: 'Cashmere Scarf', q: 1, p: 195.00 }] },
    ]);
  }
  if (!localStorage.getItem('reviews')) {
    sl('reviews', [
      { id: 1, product: 'Wool Blazer', rating: 5, text: 'Absolutely stunning quality. Fits perfectly and the material is top-notch.', date: '2026-02-20' },
      { id: 2, product: 'Silk Tie',    rating: 4, text: 'Beautiful tie, great colour. Would love more pattern options.', date: '2026-02-16' },
    ]);
  }
  if (!localStorage.getItem('userProfile')) {
    sl('userProfile', { name: 'Alex Martin', initials: 'AM' });
  }
}
...
function renderDash() {
  ...
  const active = orders.filter(o => ['processing','shipped'].includes((o.status||'').toLowerCase())).length;
  ...
  const buckets = months.map((lbl, i) => {
    ...
    const v   = orders.filter(o => o.date && o.date.startsWith(key)).reduce((s,o) => s+(o.total||0), 0);
    return { lbl, v };
  });
  ...
  // Activity
  const dotMap = { Delivered: 'dot-g', Shipped: 'dot-b', Processing: 'dot-o', Cancelled: 'dot-r' };
...
function renderOrders(orders) {
        ...
        return `<tr>
          ...
          <td>
            <div style="display:flex;gap:6px;justify-content:flex-end;flex-wrap:wrap;">
              ...
              ${s==='delivered'?`<button class="act-btn rbtn">Reorder</button>`:''}
            </div>
          </td>
        </tr>`;
      }).join('');
  ...
  document.querySelectorAll('.rbtn').forEach(b => b.addEventListener('click', () => toast('Items added to cart!', 'ok')));
}
...
function openModal(id) {
  ...
  const done = s => ['processing','shipped','delivered'].includes(s);
  $('mBody').innerHTML = `
    ...
    <div class="modal-totals">
      <div class="total-row"><span>Subtotal</span><span>$${(o.total||0).toFixed(2)}</span></div>
      <div class="total-row"><span>Shipping</span><span style="color:var(--green)">Free</span></div>
      <div class="total-row grand"><span>Total</span><span>$${(o.total||0).toFixed(2)}</span></div>
    </div>
    <div class="timeline">
...
// ── NAVIGATION ────────────────────────────────────────
const pages = {
  dashboard: { title: 'Dashboard Overview', sub: 'Welcome back — here\'s your account at a glance' },
